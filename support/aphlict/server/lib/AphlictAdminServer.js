...
JX.install('AphlictAdminServer', {
  ...
  members: {
    ...
    _onrequest: function(request, response) {
      ...
      } else if (u.pathname == '/active-clients/') {
        this._handleActiveClientsRequest(request, response, instance);
      } else {
    ...
    _handleActiveClientsRequest: function(request, response, instance) {
      var listeners = [];
      var clients = [];
      ...
      for (var ii = 0; ii < lists.length; ii++) {
        var list = lists[ii];
        listeners = listeners.concat(list.getListeners());
      }

      for (var ii = 0; ii < listeners.length; ii++) {
        var listener = listeners[ii];
        clients = clients.concat(listener._subscriptions);
      }
      ...
      response.writeHead(200, {'Content-Type': 'application/json'});
      response.write(JSON.stringify(active_clients));
      response.end();
    },
