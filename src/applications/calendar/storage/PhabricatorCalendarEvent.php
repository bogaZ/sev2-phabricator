...
final class PhabricatorCalendarEvent extends PhabricatorCalendarDAO
  ...
  public function getHost() {
    return $this->assertAttached($this->host);
  }

  public function attachHost(PhabricatorUser $host) {
    $this->host = $host;
    return $this;
  }
...
/* -(  PhabricatorConduitResultInterface  )---------------------------------- */
  ...
  public function getFieldSpecificationsForConduit() {
    return array(
      ...
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('isForDev')
        ->setType('bool')
        ->setDescription(pht('Set True for get only development event.')),
      id(new PhabricatorConduitSearchFieldSpecification())
  ...
  public function getFieldValuesForConduit() {
    ...
    if ($this->getHost()) {
      $host = $this->getHostDetail($this->getHost());
    }

    $invitees = $this->getInviteesDetail();
  ...
  private function getHostDetail($host) {
    return array(
      'userPHID' => $host->getPHID(),
      'username' => $host->getUsername(),
      'fullname' => $host->getFullName(),
      'profile_image_uri' => $host->getProfileImageURI(),
    );
  }

  private function getInviteesDetail() {
    $invitees = array();
    ...
    foreach ($this->getInvitees()  as $invitee) {
      $user = id(new PhabricatorPeopleQuery())
        ...
        ->executeOne();
      if ($user) {
        $invite['id'] = $user->getID();
        $invite['username'] = $user->getUsername();
        $invite['phid'] = $user->getPHID();
        $invite['fullname'] = $user->getFullName();
        $invite['profileImageURI'] = $user->getProfileImageURI();
        $invite['status'] = $invitee->getStatus();
        $invitees[] = $invite;
      }
    }
