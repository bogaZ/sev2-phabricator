...
final class PhabricatorCalendarEventQuery
  ...
  public function needHost($need_host) {
    $this->needHost = $need_host;
    return $this;
  }
  ...
  protected function willFilterPage(array $events) {
    ...
    if ($this->needHost) {
      $host_phids = mpull($events, 'getHostPHID');

      $hosts = id(new PhabricatorPeopleQuery())
              ->setViewer(PhabricatorUser::getOmnipotentUser())
              ->withPHIDs($host_phids)
              ->needProfile(true)
              ->needProfileImage(true)
              ->execute();

      $hosts = mgroup($hosts, 'getPHID');
      foreach ($events as $event) {
        $matched_hosts = idx($hosts, $event->getHostPHID(), array());
        $matched_host = head($matched_hosts);

        $matched_host->loadUserProfile();
        $event->attachHost($matched_host);
      }
    }
