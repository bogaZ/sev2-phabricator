...
require_once $root.'/externals/firebase-php/notify.php';
...
final class PhabricatorCalendarNotificationEngine
  ...
  private function sendNotifications() {
    ...
    foreach ($notify_map as $user_phid => $events) {
      ...
      try {
        ...
        $notif_list[] = $this->newNotifMessage($user, $events);
      } catch (Exception $ex) {
        $caught = $ex;
      }
  ...
  private function newNotifMessage(PhabricatorUser $viewer, array $events) {
    $events = msort($events, 'getEpoch');
    ...
    foreach ($events as $event) {
      ...
      $workspace = PhabricatorEnv::getEnvConfig('sev2.workspace');
        ...
        notify($event->getEvent()->getPHID(),
          $event->getEvent()->getName(),
          $message,
          array(
            'title' => $event->getEvent()->getName(),
            'body' => $message,
            'eventPHID' => $event->getEvent()->getPHID(),
            'hostPHID' => $event->getEvent()->getHostPHID(),
