...
final class PhabricatorCoursepathItemStackEditor
  ...
  public function getEditorObjectsDescription() {
    return pht('Skill Stack');
  }

  public function getCreateObjectTitle($author, $object) {
    return pht('%s created this skill stack.', $author);
  }
  ...
  public function getMailTagsMap() {
    return array(
      CoursepathTransaction::MAILTAG_DETAILS =>
        pht('Someone changes the skill stack\'s details.'),
      CoursepathTransaction::MAILTAG_COMMENT =>
        pht('Someone comments on a skill stack.'),
      CoursepathTransaction::MAILTAG_OTHER =>
        pht('Other skill stack activity not listed above occurs.'),
    );
  }
  ...
  protected function buildMailTemplate(PhabricatorLiskDAO $object) {
    $name = $object->getName();
    $id = $object->getID();
    $subject = pht('Skill stack %d: %s', $id, $name);
