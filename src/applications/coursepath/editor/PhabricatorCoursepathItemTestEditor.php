...
final class PhabricatorCoursepathItemTestEditor
  ...
  public function getCreateObjectTitle($author, $object) {
    return pht('%s created this skill test.', $author);
  }
  ...
  public function getMailTagsMap() {
    return array(
      CoursepathTransaction::MAILTAG_DETAILS =>
        pht('Someone changes the skill test\'s details.'),
      CoursepathTransaction::MAILTAG_COMMENT =>
        pht('Someone comments on a skill test.'),
      CoursepathTransaction::MAILTAG_OTHER =>
        pht('Other skill test activity not listed above occurs.'),
    );
  }
  ...
  protected function buildMailTemplate(PhabricatorLiskDAO $object) {
    $name = $object->getName();
    $id = $object->getID();
    $subject = pht('Skill Test %d: %s', $id, $name);
