...
final class DrydockLandRepositoryOperation
  ...
  public function applyOperation(
    ...
    try {
      ...
      // Run Nomad Job
      $repo_dict = $repository->toDictionary();
      $nomad_dict = $repo_dict['nomad'];

      if (!empty($nomad_dict['job'])) {
        $revisionMonogram = $revision->getMonogram();
        ...
        $urlprefix = pht('%s%s', $task->getMonogram(), $revision->getAuthorPHID());
        $image_tag = $urlprefix;

        // Prepare env
        $interface->execx(
          'echo %s > ../../.env',
          'export NOMAD_ADDR='.$nomad_dict['host']);
        $interface->execx(
          'echo %s >> ../../.env',
          'export NOMAD_TOKEN='.$nomad_dict['token']);
        $interface->execx(
          'echo %s >> ../../.env',
          'export REGION='.$nomad_dict['region']);
        $interface->execx(
          'echo %s >> ../../.env',
          'export CALLSIGN='.$repo_dict['callsign']);
        $interface->execx(
          'echo %s >> ../../.env',
          'export REVISION_ID='.$revisionMonogram);
        $interface->execx(
          'echo %s >> ../../.env',
          'export URLPREFIX='.$urlprefix);
        $interface->execx(
          'echo %s >> ../../.env',
        ...
        $interface->execx(
          'cd ../../;source .env;envsubst < %s.nomad.dist > %s.nomad;nomad job run %s.nomad',
          $revisionMonogram,
          $revisionMonogram,
          $revisionMonogram
        );
