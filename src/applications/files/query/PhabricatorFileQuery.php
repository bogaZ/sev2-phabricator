...
final class PhabricatorFileQuery
  ...
  public function needAuthor($need_author) {
    $this->needAuthor = $need_author;
    return $this;
  }
  ...
  protected function willFilterPage(array $files) {
    if ($this->needAuthor) {
      $author_phids = mpull($files, 'getAuthorPHID');

      $active_authors = id(new PhabricatorPeopleQuery())
      ...
      $disabled_authors = id(new PhabricatorPeopleQuery())
                ->setViewer(PhabricatorUser::getOmnipotentUser())
                ->withPHIDs($author_phids)
                ->withIsDisabled(true)
                ->needProfile(true)
      ...
      $authors = array_filter(array_merge($active_authors, $disabled_authors));

      $authors = mgroup($authors, 'getPHID');
      foreach ($files as $file) {
        $matched_authors = idx($authors, $file->getAuthorPHID());
        if ($matched_authors) {
          $matched_author = head($matched_authors);

          $matched_author->loadUserProfile();
          $file->attachAuthor($matched_author);
        }
      }
    }
