...
final class PhabricatorFile extends PhabricatorFileDAO
  ...
  private static function buildFromFileData($data, array $params = array()) {
    ...
    self::createFeed($file);

    return $file;
  }

  private static function createFeed($file) {
    $story_type = 'PhabricatorFileAddFeedStory';
    $story_data = array(
      'authorPHID' => $file->getAuthorPHID(),
      'objectPHID' => $file->getPHID(),
    );
    $subscribed_phids = $file->getUsersToNotifyOfTokenGiven();
    $related_phids = $subscribed_phids;
    $related_phids[] = $file->getAuthorPHID();

    id(new PhabricatorFeedStoryPublisher())
      ->setStoryType($story_type)
      ->setStoryData($story_data)
      ->setStoryTime(time())
      ->setStoryAuthorPHID($file->getAuthorPHID())
      ->setRelatedPHIDs($related_phids)
      ->setPrimaryObjectPHID($file->getPHID())
  ...
  public function getFieldValuesForConduit() {
    ...
    if ($this->getAuthorPHID()) {
      $user = id(new PhabricatorPeopleQuery())
        ...
        ->withPHIDs(array($this->getAuthorPHID()))
        ->executeOne();

      $owner['id'] = $user->getID();
      $owner['phid'] = $user->getPHID();
      $owner['username'] = $user->getUsername();
      $owner['fullname'] = $user->getFullname();
      $owner['profileImageURI'] = $user->getProfileImageURI();
    }
    ...
    return array(
      ...
      'mimeType' => $this->getMimeType(), // add mimeType
      'owner' => $owner,
      'alt' => array(
