...
final class PhabricatorFileThumbnailTransform
  ...
  const TRANSFORM_ID_DOC = 'idcard';
  const TRANSFORM_OTHER_DOC = 'other';
  ...
  public function generateTransforms() {
    return array(
      ...
      id(new self())
        ->setName(pht('ID Card (800px)'))
        ->setKey(self::TRANSFORM_ID_DOC)
        ->setScaleUp(true)
        ->setDimensions(800, null),
      id(new self())
        ->setName(pht('Other (1080px)'))
        ->setKey(self::TRANSFORM_OTHER_DOC)
        ->setScaleUp(true)
        ->setDimensions(1080, null),
    );
  }
