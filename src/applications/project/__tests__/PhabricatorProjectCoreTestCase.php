...
final class PhabricatorProjectCoreTestCase extends PhabricatorTestCase {
  ...
  public function testProjectMembersVisibility() {
    ...
    $now = PhutilCalendarAbsoluteDateTime::newFromEpoch(
      PhabricatorTime::getNow());

    $date = array($now, $now);

    $project = PhabricatorProject::initializeNewProject($user1, null, $date);
  ...
  public function testSlugMaps() {
    ...
    $project = PhabricatorProject::initializeNewProject($user, null, $date);
  ...
  public function testProjectPolicyRules() {
    ...
    $proj_a = PhabricatorProject::initializeNewProject($author, null, $date)
      ->setName('Policy A')
      ->save();
    $proj_b = PhabricatorProject::initializeNewProject($author, null, $date)
      ->setName('Policy B')
      ->save();
  ...
  private function createProject(
    ...
    $project = PhabricatorProject::initializeNewProject($user, $parent, $date);
