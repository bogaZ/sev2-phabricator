...
abstract class PhabricatorProjectController extends PhabricatorController {
  ...
  protected function addRspTags($header) {
    $project = $this->getProject();
    $viewer = $this->getViewer();

    if ($project->getIsForRsp()) {
      $rsp = id(new PHUITagView())
                ->setIcon('fa-wifi')
                ->setColor('green')
      ...
      if (!$project->hasRspSpec()) {
        ...
        $course = id(new PHUITagView())
                  ...
                  ->setName(id(new CoursepathItemQuery())
                              ->setViewer($viewer)
                              ->withPHIDs(array(
                                $project->getRspSpec()->getCoursepathItemPHID(),
                              ))
                              ->executeOne()
                              ->getName());
  ...
  protected function addDevTags($header) {
    $project = $this->getProject();
    $viewer = $this->getViewer();

    if ($project->getIsForDev()) {
  ...
  protected function renderSpecAction(PhabricatorProject $project,
                                      $via = 'profile') {
    $viewer = $this->getViewer();
    $id = $project->getID();
    ...
    $spec_icon = 'fa-wrench';
    $spec_text = pht('RSP Spec');
    $spec_href = "/project/spec/{$id}/?via=".$via;
    $spec_disabled = true;

    $spec_icon = id(new PHUIIconView())
      ->setIcon($spec_icon);
