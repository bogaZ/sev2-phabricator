...
final class PhabricatorProjectProfileController
  ...
  public function handleRequest(AphrontRequest $request) {
    ...
    $this->addRspTags($header);
    $this->addDevTags($header);
    ...
    $billing_user_list = null;

    if ($project->getIsForRsp() && $project->hasRspSpec()) {
      $billing_user_list = id(new PhabricatorProjectBillingUserListView())
        ->setUser($viewer)
        ->setProject($project)
        ->setLimit(0)
        ->setBackground(PHUIObjectBoxView::BLUE_PROPERTY)
        ->setUserPHIDs(array($project->getRspSpec()->getBillingUserPHID()));
    }
