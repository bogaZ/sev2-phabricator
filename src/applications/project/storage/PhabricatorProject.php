...
final class PhabricatorProject extends PhabricatorProjectDAO
  ...
  protected $isForRsp = 0;
  protected $isForDev = 0;
  ...
  private $rspSpec = self::ATTACHABLE;
  private $tickets = self::ATTACHABLE;
  private $columns = self::ATTACHABLE;
  ...
  public function hasRspSpec() {
    return $this->rspSpec != self::ATTACHABLE;
  }

  public function attachRspSpec(PhabricatorProjectRspSpec $rsp_spec) {
    $this->rspSpec = $rsp_spec;
    return $this;
  }

  public function getRspSpec() {
    return $this->assertAttached($this->rspSpec);
  }

  public function hasTickets() {
    return $this->tickets !== self::ATTACHABLE;
  }

  public function attachTickets(array $tickets) {
    assert_instances_of($tickets, 'PhabricatorProjectColumnPosition');
    $this->tickets = $tickets;
    return $this;
  }

  public function getTickets() {
    return $this->assertAttached($this->tickets);
  }

  public function attachColumns(array $columns) {
  ...
  public function getColumns() {
    return $this->assertAttached($this->columns);
  }
  ...
  public function updateDatasourceTokens() {
    $table = sev2table(self::TABLE_DATASOURCE_TOKEN);
  ...
  public function getTotalStoryPoints() {
    $story_points = 0;
    if ($this->getIsForRSP() && $this->hasTickets()) {
      $tickets = $this->getTickets();

      $ticket_phids = mpull($tickets, 'getObjectPHID');
      $maniphests = id(new ManiphestTaskQuery())
        ->setViewer(PhabricatorUser::getOmnipotentUser())
        ->withPHIDs($ticket_phids)
        ->execute();

      if ($maniphests) {
        $story_points = array_sum(mpull($maniphests, 'getPoints'));
      }
    }
  ...
  public function getRepository() {
    ...
    if ($edge_query) {
      ...
      if ($diff_phids) {
        foreach ($diff_phids as $phid) {
          if (strpos($phid, 'REPO') !== false) {
            $repo_phid = $phid;
          }
        }

        if ($repo_phid) {
          $diffusion_dao = new PhabricatorRepositoryURI();
          $uri = $diffusion_dao->loadOneWhere(
            'repositoryPHID = %s LIMIT 1', $repo_phid);

          $repo_uris['phid'] = $uri->getRepositoryPHID();
          $repo_uris['uri'] = $uri->getUri();
        }
      }
    }
...
/* -(  PhabricatorConduitResultInterface  )---------------------------------- */
  ...
  public function getFieldSpecificationsForConduit() {
    return array(
      ...
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('isForRsp')
        ->setType('bool')
        ->setDescription(pht('RSP enabled flag. Only has effect on rspSpec set to true')),
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('isForDev')
        ->setType('bool')
      ...
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('storyPoints')
        ->setType('int')
        ->setDescription(pht('Total Story Point of the project.')),
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('repository')
        ->setType('string')
        ->setDescription(pht('Repository info of the project.')),
    );
  }
  ...
  public function getFieldValuesForConduit() {
    ...
    // custom attributes
    $story_points = $this->getTotalStoryPoints();
    ...
    $val = array(
      ...
      'parent' => $parent_ref,
      'imageUrl' => $this->getProfileImageURI(),
      'isForRsp' => (bool)$this->getIsForRsp(),
      'isForDev' => (bool)$this->getIsForDev(),
    ...
    if ($this->isMilestone()) {
      $date_val = array(
        'startDate' => (int)$this->getUtcInitialEpoch(),
        'endDate' => (int)$this->getUtcUntilEpoch(),
      );

      if ($this->getUtcInitialEpoch() === null) {
        $date_val['startDate'] = null;
      }

      if ($this->getUtcUntilEpoch() === null) {
        $date_val['endDate'] = null;
      }

      return array_merge($val, $date_val);
    }
  ...
  public function getConduitSearchAttachments() {
    return array(
      id(new PhabricatorProjectsRspSpecSearchEngineAttachment())
        ->setAttachmentKey('rspSpec'),
...
/* -(  PhabricatorEditEngineSubtypeInterface  )------------------------------ */
  ...
  public static function newDefaultEventDateTimes($viewer_timezone, $now) {
  ...
  public function newStartDateTime() {
    $datetime = $this->getProperty('startDateTime');
    if ($datetime == null) {
      $datetime = self::getDateTimeNow('start');
      $datetime = $datetime->newAbsoluteDateTime()->toDictionary();
    }
    return $this->newDateTimeFromDictionary($datetime);
  }

  public function newEndDateTimeForEdit() {
    $datetime = $this->getProperty('endDateTime');
    if ($datetime == null) {
      $datetime = self::getDateTimeNow('end');
      $datetime = $datetime->newAbsoluteDateTime()->toDictionary();
    }
    return $this->newDateTimeFromDictionary($datetime);
  }
  ...
  private static function getDateTimeNow($when) {
    $datetime_defaults = self::newDefaultEventDateTimes(
      PhabricatorUser::getOmnipotentUser()->getTimezoneIdentifier(),
      PhabricatorTime::getNow());
    if ($when == 'start') {
      return($datetime_defaults[0]);
    }
    return($datetime_defaults[1]);
  }
}
