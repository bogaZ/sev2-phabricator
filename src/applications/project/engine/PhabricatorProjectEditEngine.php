  ...
  protected function buildCustomEditFields($object) {
    ...
    $fields = array(
      ...
      id(new PhabricatorBoolEditField())
        ...
        ->setKey('isForDev')
        ->setOptions(
          pht('Is For development'),
          pht('Is For Development Purpose'))
        ->setAsCheckbox(true)
        ->setTransactionType(
          PhabricatorProjectIsForDevTransaction::TRANSACTIONTYPE)
        ->setDescription(pht('Marks this as development testing.'))
        ->setConduitDescription(
          pht('Make this project for development testing.'))
        ->setConduitTypeDescription(
            pht('Make this project for development testing .'))
        ->setValue($object->getIsForDev()),
    );

    if ($milestone || $object->isMilestone() ||
    !($this->getController() != null)) {
      $milestone_date_fields = array(
        id(new PhabricatorEpochEditField())
          ...
          ->setDescription(pht('Start time of milestone.'))
          ->setConduitDescription(
            pht('Change the start time of the milestone.'))
          ->setConduitTypeDescription(pht('New milestone start time.'))
          ->setValue($object->newStartDateTime()->getEpoch()),
        id(new PhabricatorEpochEditField())
          ->setKey('end')
          ->setLabel(pht('End'))
          ->setIsLockable(false)
          ...
          ->setDescription(pht('End time of the milestone.'))
          ->setConduitDescription(pht('Change the end time of the milestone.'))
          ->setConduitTypeDescription(pht('New milestone end time.'))
          ->setValue($object->newEndDateTimeForEdit()->getEpoch()),
           );
       $fields = array_merge($fields, $milestone_date_fields);
    }
