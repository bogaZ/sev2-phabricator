...
final class PhabricatorProjectQuery
  ...
  public function withIsForRsp($is_for_rsp) {
    $this->isForRsp = $is_for_rsp;
    return $this;
  }
  ...
  public function needRspSpec($need_rsp_spec) {
    $this->needRspSpec = $need_rsp_spec;
    return $this;
  }

  public function needTickets($need_tickets) {
    $this->needTickets = $need_tickets;
    return $this;
  }

  public function needColumns($need_columns) {
    $this->needColumns = $need_columns;
    return $this;
  }
  ...
  protected function willFilterPage(array $projects) {
    ...
    if ($this->needRspSpec) {
      foreach ($all_graph as $phid => $project) {
        $rsp_spec = id(new PhabricatorProjectRspSpecQuery())
              ->setViewer($this->getViewer())
              ->withProjectPHIDs(array($phid))
              ->executeOne();

        if ($rsp_spec) {
          $project->attachRspSpec($rsp_spec);
        }
      }
    }
    ...
    if ($this->needTickets) {
      foreach ($all_graph as $phid => $project) {
        $tickets = id(new PhabricatorProjectColumnPositionQuery())
              ->setViewer($this->getViewer())
              ->withBoardPHIDs(array($project->getPHID()))
              ->execute();

        $tickets = mgroup($tickets, 'getBoardPHID');
        if ($tickets) {
          $project->attachTickets(idx($tickets, $project->getPHID(), array()));
        }
      }
    }
    ...
    if ($this->needColumns) {
      ...
      foreach ($all_graph as $phid => $project) {
        ...
        $project->attachColumns(idx($columns, $project->getPHID(), array()));
      }
    }
  ...
  protected function buildWhereClauseParts(AphrontDatabaseConnection $conn) {
    ...
    if ($this->phids !== null) {
      if (is_array($this->phids)
          && empty($this->phids)) {
        // Here, we deliberately return empty result
        // as the submitter attempt to query for empty
        // project phids
        $where[] = qsprintf($conn,
          'project.phid = "undefined"');
      } else {
        $where[] = qsprintf(
          $conn,
          'project.phid IN (%Ls)',
