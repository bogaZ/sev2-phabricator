...
abstract class PhabricatorController extends AphrontController {
  ...
  public function willBeginExecution() {
    ...
    if ($this->shouldRequireLogin()) {
      ...
      if (!$allow_public && !$user->isLoggedIn()) {
        if (strpos($request->getPath(),
              PhabricatorAuthFactorProviderStatus::SUITE_PATH)) {
          // This is suite app
          $login_controller = new PhabricatorAuthStartController();
          $this->setCurrentApplication($auth_application);
          return $this->delegateToController($login_controller);

        } elseif (strpos($request->getPath(),
            PhabricatorAuthExchangeIdTokenController::ID_TOKEN_PATH) !== false) {
          // This is external IDP
          $id_token_controller = new PhabricatorAuthExchangeIdTokenController();
          $this->setCurrentApplication($auth_application);
          return $this->delegateToController($id_token_controller);
        } else {
          // Normal login
          $login_controller = new PhabricatorAuthStartController();
          $this->setCurrentApplication($auth_application);
    ...
    // NOTE : recheck this one impact on wider system
    // if ($this->shouldRequireLogin() && !$user->getIsAdmin()) {
    // if ($this->shouldRequireLogin()) {
      // return new Aphront403Response();
    // }

    // try {
    //   if ($this->isNotWhitelistedPaths($request)) {
    // Other than /suite &/ conduit, ignore non org member to access the system
    //     if ($this->isNotOrganizationMember($user)
    //         && !$user->getIsConnect()) {
    //       return $this->enforceOrganizationPolicy($request);
    //     }
    //   }
    // } catch (Exception $e) {}
  }
  ...
  public function newSuitePage() {
    $page = id(new PhabricatorSuitePageView())
...
/* -(  Internal  )--------------------------------------------------------- */
  protected function isNotOrganizationMember(PhabricatorUser $user) {
    try{
      $base_uri = PhabricatorEnv::getEnvConfig('phabricator.base-uri');
      return strpos($base_uri, PhabricatorAuthFactorProviderStatus::ORG_DOMAIN) !== false
        && strpos($user->loadPrimaryEmailAddress(), PhabricatorAuthFactorProviderStatus::ORG_EMAIL) === false;
    } catch (Exception $e) {
      return false;
    }
  }

  protected function isNotWhitelistedPaths($request) {
    return strpos($request->getPath(), PhabricatorAuthFactorProviderStatus::SUITE_PATH) === false;
  }

  protected function enforceOrganizationPolicy(AphrontRequest $request) {
    $viewer = $this->getViewer();

    if ($viewer->isLoggedIn()) {
      $suite_class = 'PhabricatorSuiteApplication';
      $suite_installed = PhabricatorApplication::isClassInstalledForViewer(
        $suite_class,
        $viewer);
      if ($suite_installed) {
        $application = PhabricatorApplication::getByClass($suite_class);
        $this->setCurrentApplication($application);

        $controller = new PhabricatorSuiteCheckpointController();
        return $this->delegateToController($controller);
      }
    }
  }
