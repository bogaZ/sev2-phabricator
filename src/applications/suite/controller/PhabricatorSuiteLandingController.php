...
final class PhabricatorSuiteLandingController
  ...
  public function handleRequest(AphrontRequest $request) {
    // Mark this request as coming from suite
    PhabricatorCookies::setSuiteCookie($request);
    ...
    if (!$request->isFormPost()) {
      if (strlen($next_uri)) {
        PhabricatorCookies::setSuiteNextURICookie($request, $next_uri);
      }
    ...
    if ($viewer->isLoggedIn()) {
      ...
      return id(new AphrontRedirectResponse())->setURI('/suite/auth/finish');
    }
    ...
    if ($are_buttons) {
      ...
      $out[] = phutil_tag(
        'div',
        array(
          'class' => 'phabricator-login-buttons centered',
        ),
        $button_columns);
    }
    ...
    $suite_icon = id(new PHUIBigInfoView())
      ->setTitle(new PhutilSafeHTML('<h1>Suite</h1>'))
      ->setDescription($this->buildSuiteMessage());

    $suite_footer = phutil_tag(
      'div',
      array(
        'class' => 'phabricator-standard-page-footer grouped centered',
      ),
      new PhutilSafeHTML('&copy;  2016 - '.date('Y').
        ' Refactory - All Rights Reserved'));

    return $this->newSuitePage()
      ->setTitle($title)
      ->setSplash(true)
      ->appendChild($suite_icon)
      ->appendChild($view)
      ->appendChild($suite_footer);
  }
  ...
  private function processAjaxRequest() {
    ...
    return $this->newDialog()
      ->setTitle(pht('Dibutuhkan autentikasi'))
      ->appendParagraph(pht('Masuk untuk mengambil langkah ini.'))
      ->addSubmitButton(pht('Masuk'))
      ->addCancelButton('/');
  }
  ...
  protected function buildSuiteMessage() {
    return phutil_tag(
      'div',
      array(
        'class' => 'suite-heading centered justify-vertical',
      ),
      new PhutilSafeHTML('<h1>Empowering <span>People</span> Through Programming</h1>'));
  }
}
