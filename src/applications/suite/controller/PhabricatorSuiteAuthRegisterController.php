...
final class PhabricatorSuiteAuthRegisterController
  ...
  public function handleRequest(AphrontRequest $request) {
    ...
    if ($try_register) {
      ...
      if (!$errors) {
        ...
        try {
          ...
          $user->setIsSuite(1);
          $user->setIsSuiteSubscribed(1);
          $user->setIsSuiteOnline(1);
    ...
    if ($is_setup) {
      ...
      $title = pht('Welcome to Suite');
    } else {
      $crumbs->addTextCrumb(pht('Register'));
      $crumbs->addTextCrumb($provider->getProviderName());
      $title = pht('Create a Suite Account');
    }
