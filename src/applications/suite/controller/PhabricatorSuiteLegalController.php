...
final class PhabricatorSuiteLegalController extends PhabricatorController {
  ...
  public function handleRequest(AphrontRequest $request) {
    ...
    switch ($document->getSignatureType()) {
      ...
      case LegalpadDocument::SIGNATURE_TYPE_INDIVIDUAL:
        if ($signer_phid) {
          // NOTE: This is odd and should probably be adjusted after
    ...
    if ($request->isFormOrHisecPost() && !$has_signed) {
      ...
      if (!$errors) {
        ...
        if ($viewer->isLoggedIn() && $is_individual) {
          $next_uri = '/suite/legal/'.$document->getMonogram();
