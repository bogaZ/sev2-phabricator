...
final class PhabricatorRepository extends PhabricatorRepositoryDAO
  ...
  public function toDictionary() {
    return array(
      ...
      'nomad' => array(
        'host' => $this->getNomadHost(),
        'token' => $this->getNomadToken(),
        'region' => $this->getNomadRegion(),
        'job' => $this->getNomadJobDecoded(),
      ),
    );
  }
  ...
  public function getNomadAttr($key, $default = null) {
    if ($this->nomad == null) {
      $this->nomad = array();
    }

    return idx($this->nomad, $key, $default);
  }

  public function setNomadAttr($key, $value) {
    if ($this->nomad == null) {
      $this->nomad = array();
    }

    $this->nomad[$key] = $value;
    return $this;
  }
  ...
  public function delete() {
    $this->openTransaction();
      ...
      queryfx(
        ...
        sev2table(self::TABLE_FILESYSTEM),
        $this->getID());

      queryfx(
        $conn_w,
        'DELETE FROM %T WHERE repositoryID = %d',
        sev2table(self::TABLE_PATHCHANGE),
        $this->getID());

      queryfx(
        $conn_w,
        'DELETE FROM %T WHERE repositoryID = %d',
        sev2table(self::TABLE_SUMMARY),
        $this->getID());
...
/* -(  Nomad  )------------------------------------------------------------ */
  ...
  public function getNomadHost() {
    return $this->getNomadAttr('host', null);
  }


  public function getNomadToken() {
    return $this->getNomadAttr('token', null);
  }

  public function getNomadRegion() {
    return $this->getNomadAttr('region', null);
  }

  public function getNomadJob() {
    return $this->getNomadAttr('job', null);
  }

  public function getNomadJobDecoded() {
    return base64_decode($this->getNomadJob());
  }
