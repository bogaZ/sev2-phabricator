...
final class PhabricatorCookies extends Phobject {
   ...
   * Stores Suite Flag
   */
  const COOKIE_SUITE = 'suite';

  /**
   * Stores Suite Flag
   */
  const COOKIE_SUITE_NEXTURI = 'suite_next_uri';
...
/* -(  Suite Cookie  )--------------------------------------------------- */
  ...
  /**
   * Set the Suite cookie. This is a random cookie used like a CSRF value
   * during authentication workflows.
   *
   * @param AphrontRequest  Request to modify.
   * @return void
   * @task clientid
   */
  public static function setSuiteCookie(AphrontRequest $request) {
    $value = $request->getCookie(self::COOKIE_SUITE);
    if (!strlen($value)) {
      $request->setTemporaryCookie(
        self::COOKIE_SUITE,
  ...
  /**
   * Set the Suite Next URI cookie. We only write the cookie if it wasn't recently
   * written, to avoid writing over a real URI with a bunch of "humans.txt"
   * stuff. See T3793 for discussion.
   *
   * @param   AphrontRequest    Request to write to.
   ...
   * @task next
   */
  public static function setSuiteNextURICookie(
    AphrontRequest $request,
    $next_uri,
    $force = false) {

    if (!$force) {
      $cookie_value = $request->getCookie(self::COOKIE_SUITE_NEXTURI);
      list($set_at, $current_uri) = self::parseSuiteNextURICookie($cookie_value);
    ...
    $new_value = time().','.$next_uri;
    $request->setTemporaryCookie(self::COOKIE_SUITE_NEXTURI, $new_value);
  }


  /**
   * Read the Suite URI out of the Next URI cookie.
   *
   * @param   AphrontRequest  Request to examine.
   * @return  string|null     Next URI cookie's URI value.
   *
   * @task next
   */
  public static function getSuiteNextURICookie(AphrontRequest $request) {
    $cookie_value = $request->getCookie(self::COOKIE_SUITE_NEXTURI);
    list($set_at, $next_uri) = self::parseSuiteNextURICookie($cookie_value);
  ...
  /**
   * Parse a Suite Next URI cookie into its components.
   *
   * @param   string        Raw cookie value.
   * @return  list<string>  List of timestamp and URI.
   *
   * @task next
   */
  private static function parseSuiteNextURICookie($cookie) {
    // Old cookies look like: /uri
    // New cookies look like: timestamp,/uri
