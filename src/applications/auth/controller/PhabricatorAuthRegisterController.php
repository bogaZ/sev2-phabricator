    ...
    if ($try_register) {
      ...
      if (!$errors) {
        ...
        try {
          ...
          $approved = $must_approve
                && PhabricatorUser::isNotRefactorian($value_email)
                && !PhabricatorUser::isWorkspaceAdmin($value_email);
          $user->setIsApproved((int)!$approved);

          if (PhabricatorUser::isWorkspaceAdmin($value_email)) {
            $user->setIsAdmin(1);
          }
  ...
  private function loadDefaultAccount($invite) {
    ...
    if (!$providers) {
      $response = $this->renderError(
        pht(
          'Untuk masuk ke kantor digital, '.
          'gunakan aplikasi desktop atau mobile'));
