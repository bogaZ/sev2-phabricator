...
final class PhabricatorAuthLoginController
    ...
    if ($account->getUserPHID()) {
      ...
      if (!$viewer->isLoggedIn()) {
        if ($provider->shouldAllowRegistration() || $invite) {
          if ($request->getCookie(PhabricatorCookies::COOKIE_SUITE)) {
            return $this->processRegisterSuiteUser($account);
          } else {
  ...
  private function processRegisterSuiteUser(PhabricatorExternalAccount $account) {
    $account_secret = $account->getAccountSecret();

    $suite_class = 'PhabricatorSuiteApplication';
    $suite_application = PhabricatorApplication::getByClass($suite_class);

    $suiteAuthRegister = new PhabricatorSuiteAuthRegisterController();
    $suiteAuthRegister->setCurrentApplication($suite_application);
    $register_uri = $suiteAuthRegister->getApplicationURI('auth/register/'.$account_secret.'/');
