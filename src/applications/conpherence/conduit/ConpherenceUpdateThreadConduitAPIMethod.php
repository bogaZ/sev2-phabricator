...
final class ConpherenceUpdateThreadConduitAPIMethod
  ...
  protected function defineErrorTypes() {
    return array(
      'ERR_UNATHORIZED' => pht(
        'You must have edit capabilities.'),
  ...
  protected function execute(ConduitAPIRequest $request) {
    ...
    if (count($add_participant_phids) > 0
      || ($remove_participant_phid
        && $remove_participant_phid != $user->getPHID())) {
    ...
    $query->requireCapabilities($needed_capabilities);
