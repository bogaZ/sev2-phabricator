...
final class ConpherenceEditEngine
  ...
  protected function buildCustomEditFields($object) {
    ...
    // Create new feature to add project into Conpherence so it can be group
    // by active or chat.
    $tags_phid = null;
    if ($object->getTagsPHID()) {
      $tags_phid = array($object->getTagsPHID());
    }
    return array(
      ...
      id(new PhabricatorProjectsEditField())
        ->setKey('tags')
        ->setLabel(pht('Tags'))
        ...
        ->setTransactionType(
          ConpherenceThreadTagsTransaction::TRANSACTIONTYPE)
        ->setSingleValue(1)
        ->setValue($tags_phid)
        ->setViewer($viewer),
