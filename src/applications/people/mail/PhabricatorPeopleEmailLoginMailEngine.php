...
final class PhabricatorPeopleEmailLoginMailEngine
  ...
  protected function newMail() {
    ...
    if ($is_set_password) {
      $subject = pht('[Suite] Account Password Link');
    } else {
      $subject = pht('[Suite] Account Login Link');
    }
    ...
    if ($have_passwords) {
      if ($is_set_password) {
        ...
        $body[] = pht(
          "You can use this link to reset your Refactory Dashboard password:".
          "\n\n  %s\n",
          $login_uri);
      } else {
        $body[] = pht(
      ...
      $body[] = pht(
        "You can use this login link to regain access to your Refactory Dashboard ".
        "account:".
        "\n\n".
        "  %s\n",
        $login_uri);
