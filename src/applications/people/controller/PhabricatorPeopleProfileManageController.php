...
final class PhabricatorPeopleProfileManageController
  ...
  private function buildCurtain(PhabricatorUser $user) {
    ...
    foreach ($sev2_roles_map as $key => $value) {
      if (in_array($key, $obj)) {
        $roles_icon[] = 'fa-arrow-circle-o-down';
        $roles_name[] = pht('Remove %s', $value);
      } else {
        $roles_icon[] = 'fa-arrow-circle-o-up';
        $roles_name[] = pht('Make %s', $value);
      }
    }
    foreach ($roles_icon as $key => $value) {
      $curtain->addAction(
        id(new PhabricatorActionView())
          ->setIcon($value)
          ->setName($roles_name[$key])
          ->setDisabled(!$is_admin)
          ->setWorkflow(true)
          ->setHref($this->getApplicationURI('roles/'.$id.'/'.
          array_keys($sev2_roles_map)[$key].'/')));
    }
