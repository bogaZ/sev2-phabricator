...
final class PhabricatorPeopleProfileViewController
  ...
  public function handleRequest(AphrontRequest $request) {
    ...
    $lobby = Lobby::buildAvailabilityGraph($user);
    ...
    $suite = $this->buildSuiteView($user);
  ...
  private function buildSuiteView(
    PhabricatorUser $user) {

    // Only continue if current user is registered through Suite
    if (!$user->getIsSuite()) {
      return null;
    }
    ...
    $header = id(new PHUIHeaderView())
      ->setHeader(pht('Suite Profile'));

    $suite = id(new SuiteProfileView())
              ->setViewer($viewer)
              ->setSuiteUser($user);
