...
final class PhabricatorPeopleNewController
  ...
  public function handleRequest(AphrontRequest $request) {
    ...
    if ($is_bot) {
      ...
      $title = pht('Create New Connect User');
      $form->appendRemarkupInstructions(
        pht('You are creating a new **connect** user account.'));
    } else {
    ...
    if ($is_connect) {
      $form
        ...
        ->appendRemarkupInstructions(
          pht(
            '**What is connect user?**'.
            "\n\n".
            'We are enforcing organization policy throughout dashboard, '.
            'so if someone attempt to register using non `@refactory.id` '.
            'email address, they would not be able to login. But we might '.
            'need to create user that able to '.
            'login without organization email.'.
            ' Examples include:'.
            "\n\n".
            "  - Client.\n".
            "  - External Client Developer.\n".
            "  - Premium RSP who needs to have access to dashboard.\n".
            "  - Some internal use cases.\n".
            "\n\n".
            "All connect user will have {icon id-card-o} icon next to it."));
    }
