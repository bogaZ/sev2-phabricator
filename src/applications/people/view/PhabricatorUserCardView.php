...
final class PhabricatorUserCardView extends AphrontTagView {
  ...
  protected function getTagContent() {
    ...
    if (id(new Lobby())->setViewer($user)->allowedTojoin()) {
      $cs = PhabricatorContentSource::newForSource(
        LobbyContentSource::SOURCECONST);
      $lobby = LobbyState::getCurrent($user, $cs);

      $current_task = $lobby->getCurrentTask();
      if (empty($current_task)) {
        $maps = LobbyState::getStatusMap();
        $const = $lobby->getStatus();

        if (isset($maps[$const])) {
          $current_task = $maps[$const];
        } else {
          $current_task = LobbyState::DEFAULT_TASK;
        }
      }
