...
final class PhabricatorPeopleQuery
  ...
  private $isForDev;
  private $isSuite;
  private $isSuiteOnline;
  private $isSuiteSubscribed;
  private $isSuiteDisabled;
  private $isDisabled;
  private $isConnect;
  ...
  public function withIsForDev($for_dev) {
    $this->isForDev = $for_dev;
    return $this;
  }

  public function withIsSuite($suite) {
    $this->isSuite = $suite;
    return $this;
  }

  public function withIsSuiteSubscribed($suite_subscribed) {
    $this->isSuiteSubscribed = $suite_subscribed;
    return $this;
  }

  public function withIsSuiteDisabled($suite_disabled) {
    $this->isSuiteDisabled = $suite_disabled;
    return $this;
  }

  public function withIsSuiteOnline($suite_online) {
    $this->isSuiteOnline = $suite_online;
    return $this;
  }

  public function withIsConnect($connect) {
    ...
    if ($this->nameTokens) {
      foreach ($this->nameTokens as $key => $token) {
        ...
        $joins[] = qsprintf(
          ...
          sev2table(PhabricatorUser::NAMETOKEN_TABLE),
          $token_table,
          $token_table,
          $token_table,
          $token);
  ...
  protected function buildWhereClauseParts(AphrontDatabaseConnection $conn) {
    ...
    if ($this->isSuiteSubscribed !== null) {
      $where[] = qsprintf(
        $conn,
        'user.isSuiteSubscribed = %d',
        (int)$this->isSuiteSubscribed);
    }

    if ($this->isSuiteDisabled !== null) {
      $where[] = qsprintf(
        $conn,
        'user.isSuiteDisabled = %d',
        (int)$this->isSuiteDisabled);
    }

    if ($this->isSuiteOnline !== null) {
      $where[] = qsprintf(
        $conn,
        'user.isSuiteOnline = %d',
        (int)$this->isSuiteOnline);
    }
