...
final class PhabricatorUser
  ...
  protected $isConnect = 0;

  protected $isSuite = 0;
  protected $isSuiteSubscribed = 0;
  protected $isSuiteDisabled = 0;
  protected $isSuiteOnline = 0;
  protected $suiteVersion;
  ...
  protected function readField($field) {
    switch ($field) {
      ...
      case 'isSuite':
        return (bool)$this->isSuite;
      case 'isConnect':
        return (bool)$this->isConnect;
      case 'isSuiteSubscribed':
        return (bool)$this->isSuiteSubscribed;
      case 'isSuiteDisabled':
        return (bool)$this->isSuiteDisabled;
      case 'isSuiteOnline':
        return (bool)$this->isSuiteOnline;
      default:
  ...
  public function isUserActivated() {
    ...
    if ($this->getIsDisabled() || $this->getIsSuiteDisabled()) {
      return false;
    }
  ...
  public function isResponsive() {
    ...
    if ($this->readField('isSuite')) {
      return $this->readField('isSuiteOnline');
    } else {
      return true;
    }
  }
  ...
  protected function getConfiguration() {
    return array(
      ...
      self::CONFIG_COLUMN_SCHEMA => array(
        ...
        'isSuite' => 'bool',
        'jid' => 'text255?',
        'isSuiteDisabled' => 'bool',
        'isSuiteOnline' => 'bool',
        'isSuiteSubscribed' => 'bool',
        'customRoles' => 'text?',
        'suiteVersion' => 'text20?',
...
/* -(  Settings  )----------------------------------------------------------- */
  ...
  public function updateNameTokens() {
    $table  = sev2table(self::NAMETOKEN_TABLE);
  ...
  public function getProjectGroupPHIDs() {
...
/* -(  PhabricatorConduitResultInterface  )---------------------------------- */
  ...
  public function getFieldSpecificationsForConduit() {
    return array(
      ...
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('profileImageURI')
        ->setType('string')
        ->setDescription(pht("The user's profile picture .")),
    );
  }
  ...
  public function getFieldValuesForConduit() {
    ...
    if ($lobby_state) {
      ...
      $device = $lobby_state->getDevice();
    }

    return array(
      'username' => $this->getUsername(),
      'realName' => $this->getRealName(),
      'primaryEmail' => $this->loadPrimaryEmailAddress(),
      'device' => $device,
      'roles' => $roles,
      'profileImageURI' => $this->getProfileImageURI(),
      'state' => $state,
      'groupPHIDs' => $this->getProjectGroupPHIDs(),
      'jid' => $this->getJid(),
    );
  }

  private function getLobbyStateDetail() {
    $lobby_state = id(new LobbyStateQuery())
      ->setViewer($this)
      ->withOwnerPHIDs(array($this->getPHID()))
      ->executeOne();
...
/* -(  PhabricatorAuthPasswordHashInterface  )------------------------------- */
  ...
  /**
   * SEV-2 & Refactory internal logic
   */

  /**
   * Check if given user isn't refactorian
   * @param string email
   * @return bool
   */
  public static function isNotRefactorian($email) {
    return strpos($email, 'refactory') === false;
  }

  /**
   * Check if given user is workspace admin
   *
   * @param string email
   * @return bool
   */
  public static function isWorkspaceAdmin($email) {
    return $email == PhabricatorEnv::getEnvConfig('sev2.admin-email');
  }
