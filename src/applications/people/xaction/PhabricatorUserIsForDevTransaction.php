...
final class PhabricatorUserIsForDevTransaction
  extends PhabricatorUserTransactionType {

  const TRANSACTIONTYPE = 'user.is-for-dev';

  public function generateOldValue($object) {
    return (bool)$object->getIsForDev();
  }
  ...
  public function getTitle() {
    ...
    if ($new) {
      return pht(
        '%s mark this user as dev user.',
        $this->renderAuthor());
    } else {
      return pht(
        '%s mark this user as non-dev user.',
        $this->renderAuthor());
    }
  }
  ...
  public function validateTransactions($object, array $xactions) {
    ...
    foreach ($xactions as $xaction) {
      $is_disabled = (bool)$object->getIsForDev();
      ...
      if ($this->getActingAsPHID() === $object->getPHID()) {
        $errors[] = $this->newInvalidError(
          pht('You can not mark your own account.'));
      }
    }
