...
final class PhortuneMerchantQuery
  ...
  public static function canViewersEditMerchants(
    ...
    if (!empty($load_phids)) {
      $potential_admin = id(new PhabricatorPeopleQuery())
                          ->setViewer(PhabricatorUser::getOmnipotentUser())
                          ->withPHIDs($load_phids)
                          ->executeOne();
      if ($potential_admin->getIsAdmin()) {
        $all_merchants = id(new self())
          ->setViewer(PhabricatorUser::getOmnipotentUser())
          ->execute();
        $refactory = head($all_merchants);
        $admin_phid = $potential_admin->getPHID();
        $cache_data[$admin_phid][$refactory->getPHID()] = true;
      }
    }
