...
final class PhortuneAccountQuery
  ...
  public static function loadAccountsForUser(
    ...
    // Ensure balance
    if ($create_default_balance) {
      $new_account = head($accounts);
      SuiteBalancePaymentProvider::loadBalanceMethod(
        $user,
        $new_account,
        $content_source);
    ...
    SuiteBalanceQuery::loadBalanceForUserAccount($user,
        $new_account, $content_source);
