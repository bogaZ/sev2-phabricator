...
final class ManiphestTaskQuery extends PhabricatorCursorPagedPolicyAwareQuery {
  ...
  const ORDER_POINTS        = 'order-points';
  ...
  public function withTitlePrefixes(array $prefixes) {
    $this->titlePrefixes = $prefixes;
    return $this;
  }
  ...
  public function withOwnersQA(array $owners) {
    if ($owners === array()) {
      throw new Exception(pht('Empty withOwnersQA() constraint is not valid.'));
    }
  ...
  public function withProgresses($progress) {
  ...
  public function needViewer(PhabricatorUser $user) {
    $this->needViewer = $user;
    return $this;
  }
  ...
  public function withTitleLike($like) {
    $this->titleLike = $like;
    return $this;
  }

  public function withColumnName($column_name) {
  ...
  protected function didFilterPage(array $tasks) {
    ...
    if ($this->needViewer) {
      foreach ($tasks as $task) {
        $task->setViewer($this->needViewer);
      }
    }
    return $tasks;
  }
  ...
  protected function buildWhereClauseParts(AphrontDatabaseConnection $conn) {
    ...
    if (!is_null($this->progress) && is_numeric($this->progress)) {
      $where[] = qsprintf(
        $conn,
        'task.progress = %s',
        $this->progress);
    } else if (is_string($this->progress)) {

      $progress = preg_replace('/[\<\>\=|\s]*/', '', $this->progress);
      $progress = (int)trim($progress);

      if (preg_match('/\<\s?\d/', $this->progress)) {
        $where[] = qsprintf(
          $conn,
          'task.progress < %d',
          $progress);
      } else if (preg_match('/\<=\s?\d/', $this->progress)) {
        $where[] = qsprintf(
          $conn,
          'task.progress <= %d',
          $progress);
    ...
    if ($this->titlePrefixes) {
      $parts = array();
      foreach ($this->titlePrefixes as $title_prefix) {
        $parts[] = qsprintf(
          $conn,
          'task.title LIKE %>',
          $title_prefix);
    ...
    // Add function to generete all column phid to find
    // current state of all task
    if ($this->columnName !== null && $this->columnName !== 'All') {
        $column_name = $this->columnName;
        $phids = array();
        $maniphest_transaction  = id(new ManiphestTransaction());
        $conn_mn =  $maniphest_transaction->establishConnection('r');

        $column  = id(new PhabricatorProjectColumn());
        $con_mn =  $column->establishConnection('r');

        $columns = queryfx_all(
          $con_mn,
          'SELECT phid from %T where name =  %s',
          sev2table($column->getTableName()),
          $column_name);

        if (!empty($columns)) {
          $phids = queryfx_all(
            $conn_mn,
            'SELECT objectPHID as phid
            FROM %T t
            WHERE transactionType = %s
            AND t.id = ( SELECT  max(mt.id)
            FROM %T mt
            WHERE JSON_EXTRACT(mt.newValue->"$[0]","$.boardPHID") =
            JSON_EXTRACT(t.newValue->"$[0]","$.boardPHID")
            AND mt.objectPHID = t.objectPHID
            AND mt.transactionType = %s)
            AND TRIM(BOTH %s FROM
            JSON_EXTRACT(t.newValue->"$[0]","$.columnPHID")) IN
            (%Ls)',
            sev2table($maniphest_transaction->getTableName()),
            'core:columns',
            sev2table($maniphest_transaction->getTableName()),
            'core:columns',
            '"',
            array_column($columns, 'phid'));
        }
      // $phids = [['phid' => 'PHID-TASK-NONE']];
      // this code use for error handling
      // so when none ticket on that state it will return no task found
      if (empty($phids)) {
        $phids = [['phid' => 'PHID-TASK-NONE']];
      }
  ...
  private function buildOwnerWhereClause(AphrontDatabaseConnection $conn) {
    ...
    if ($this->ownerPHIDs !== null &&
        $this->ownerQAPHIDs !== null) {
          $subclause[] = qsprintf(
            $conn,
            'task.ownerPHID IN (%Ls) AND
              task.ownerQAPHID IN (%Ls)',
            $this->ownerPHIDs,
            $this->ownerQAPHIDs);
    } else {
      if ($this->ownerPHIDs !== null) {
  ...
  protected function buildJoinClauseParts(AphrontDatabaseConnection $conn) {
    ...
    $edge_table = PhabricatorEdgeConfig::getEdgeTableName();
    ...
    if ($this->subscriberPHIDs !== null) {
      $joins[] = qsprintf(
        ...
        'AND e_ccs.type = %s '.
        'AND e_ccs.dst in (%Ls)',
        PhabricatorEdgeConfig::getEdgeTableName(),
  ...
  public function getBuiltinOrders() {
    $orders = array(
      ...
      'points' => array(
        'vector' => array('points', 'id'),
        'name' => pht('Points'),
        'aliases' => array(self::ORDER_POINTS),
      ),
      'updated' => array(
