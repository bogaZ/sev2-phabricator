...
final class ManiphestTaskSearchEngine
  ...
  const COLUMN_TYPE_BACKLOG = 0;
  const COLUMN_TYPE_TICKET_IN_REVIEW = 1;
  const COLUMN_TYPE_TO_DO = 2;
  const COLUMN_TYPE_REJECTED = 3;
  const COLUMN_TYPE_IN_PROGRESS = 4;
  const COLUMN_TYPE_BLOCKED = 5;
  const COLUMN_TYPE_CODE_REVIEW_INTERNAL = 6;
  const COLUMN_TYPE_CODE_REVIEW_EXTERNAL = 7;
  const COLUMN_TYPE_CODE_READY_TO_TEST = 8;
  const COLUMN_TYPE_IN_TESTING_INTERNAL = 9;
  const COLUMN_TYPE_IN_TESTING_EXTERNAL = 10;
  const COLUMN_TYPE_DONE = 11;
  ...
  protected function buildCustomSearchFields() {
    ...
    return array(
      ...
      id(new PhabricatorOwnersSearchField())
        ->setLabel(pht('Assigned QA To'))
        ->setKey('assignedQAPHIDs')
        ->setConduitKey('assignedQA')
        ->setAliases(array('assignedQA'))
        ->setDescription(
          pht('Search for tasks QA owned by a user from a list.')),
      ...
      id(new PhabricatorSearchTextField())
        ->setLabel(pht('Id Like'))
        ->setKey('idLike')
        ->setAliases(array('idLike'))
        ->setDescription(
          pht('Search for tasks with constraint idLike.')),
      ...
      id(new PhabricatorSearchTextField())
        ...
        ->setKey('progress')
        ->setAliases(array('progress'))
        ->setDescription(
          pht('Search for tasks with current progress.')),
      id(new PhabricatorSearchSelectField())
        ...
        ->setDefault('All'),
      id(new PhabricatorSearchTextField())
        ->setLabel(pht('Title Contains'))
        ->setKey('titleLike')
        ->setAliases(array('titleLike'))
        ->setDescription(
          pht('Find manifests whose titles contain a substring.')),
      id(new PhabricatorSearchDatasourceField())
  ...
  protected function buildQueryFromParameters(array $map) {
    ...
    if ($map['assignedQAPHIDs']) {
      $query->withOwnersQA($map['assignedQAPHIDs']);
    }
    ...
    if (!is_null($map['progress'])) {
      $query->withProgresses($map['progress']);
    }
    ...
    if ($map['titleLike']) {
      $query->withTitleLike($map['titleLike']);
    }
    ...
    if ($map['columnNames']) {
      $value = ltrim($map['columnNames']);
      $query->withColumnName($value);
    }
  ...
  private function getStateOptions() {
    return array(
      'Ticket In Review' => pht('Ticket In Review'),
      'To Do' => pht('To Do'),
      'Rejected' => pht('Rejected'),
      'In Progress' => pht('In Progress'),
      'Blocked' => pht('Blocked'),
      'Code Review Internal' => pht('Code Review Internal'),
      'Code Review External' => pht('Code Review External'),
      'Ready To Test' => pht('Ready To Test'),
      'In Testing Internal' => pht('In Testing Internal'),
      'In Testing External' => pht('In Testing External'),
      'Done' => pht('Done'),
      'All' => pht('(Show All)'),
    );
  }

  private function getStateKeys($map) {
    $key = 100;
    if ($map === 'Ticket In Review') {
      $key = self::COLUMN_TYPE_TICKET_IN_REVIEW;
    } else if ($map === 'To Do') {
      $key = self::COLUMN_TYPE_TO_DO;
    } else if ($map === 'Rejected') {
      $key = self::COLUMN_TYPE_REJECTED;
    } else if ($map === 'In Progress') {
      $key = self::COLUMN_TYPE_IN_PROGRESS;
    } else if ($map === 'Blocked') {
      $key = self::COLUMN_TYPE_BLOCKED;
    } else if ($map === 'Code Review Internal') {
      $key = self::COLUMN_TYPE_CODE_REVIEW_INTERNAL;
    } else if ($map === 'Code Review External') {
      $key = self::COLUMN_TYPE_CODE_REVIEW_EXTERNAL;
    } else if ($map === 'Ready To Test') {
      $key = self::COLUMN_TYPE_CODE_READY_TO_TEST;
    } else if ($map === 'In Testing Internal') {
      $key = self::COLUMN_TYPE_IN_TESTING_INTERNAL;
    } else if ($map === 'In Testing External') {
      $key = self::COLUMN_TYPE_IN_TESTING_EXTERNAL;
    } else if ($map === 'Done') {
      $key = self::COLUMN_TYPE_DONE;
    }
    return $key;
  }
  ...
  protected function renderResultList(
    ...
    $param_result = $saved->getParameters();
    $task_list = array();
  ...
  protected function newExportFields() {
    ...
    if (ManiphestTaskPointsQA::getIsEnabled()) {
      $fields[] = id(new PhabricatorDoubleExportField())
        ->setKey('pointsQA')
        ->setLabel('Points QA');
    }
  ...
  protected function newExportData(array $tasks) {
    ...
    foreach ($tasks as $task) {
      ...
      $task_position = '';
      $position = $task->getColumnPosition($task, $viewer);
      if ($position) {
