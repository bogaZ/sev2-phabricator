...
final class ManiphestTaskDetailController extends ManiphestController {
  ...
  private function buildHeaderView(ManiphestTask $task) {
    ...
    if ($field_list) {
      foreach ($field_list->getFields() as $key => $value) {
        $field = explode(':', $key);
        if (end($field) == 'reviewerPoints' &&
            !is_null($value->getProxy()->getFieldValue())) {
              $reviewer_point = $value->getProxy()->getFieldValue();
        }
      }
    }
    ...
    if (ManiphestTaskPoints::getIsEnabled()) {
      $is_for_rsp = false;
      $related_projects = id(new PhabricatorProjectQuery())
                          ->withPHIDs($task->getProjectPHIDs())
                          ->needRspSpec(true)
                          ->setViewer($this->getViewer())
                          ->execute();

      foreach ($related_projects as $related_project) {
        if ($related_project->getIsForRsp()
            && $related_project->hasRspSpec()) {
          $is_for_rsp = true;
          $rsp_tag = id(new PHUITagView())
            ->setIcon('fa-wifi')
            ->setName('RSP enabled')
      ...
      if ($points !== null) {
        ...
        if ($is_for_rsp) {
          // Display warning
          $spec = id(new PHUITagView())
                ->setIcon('fa-ban')
                ->setColor('yellow')
                ->setType(PHUITagView::TYPE_OBJECT)
                ->setName('Missing story point');
          $view->addTag($spec);
        }
      }

      if ($reviewer_point != null) {
        $points_name = pht('%s %s',
          $reviewer_point,
          'Reviewer Points');
        $tag = id(new PHUITagView())
          ->setName($points_name)
          ->setColor(PHUITagView::COLOR_SKY)
          ->setType(PHUITagView::TYPE_SHADE);
  ...
  private function buildCurtain(
    ...
    $owner_phid_qa = $task->getOwnerQAPHID();
    $author_phid = $task->getAuthorPHID();
    $handles = $viewer->loadHandles(
      array($owner_phid, $author_phid, $reviewer_phid));
    $handles_qa = $viewer->loadHandles(array($owner_phid_qa, $author_phid));
    ...
    $assigned_refs_qa = id(new PHUICurtainObjectRefListView())
      ->setViewer($viewer)
      ->setEmptyMessage(pht('None'));

    if ($owner_phid_qa) {
        $assigned_ref_qa = $assigned_refs_qa->newObjectRefView()
          ->setHandle($handles_qa[$owner_phid_qa])
          ->setHighlighted($owner_phid_qa === $viewer_phid);
    }

    $curtain->newPanel()
      ->setHeaderText(pht('Assigned To QA'))
      ->appendChild($assigned_refs_qa);

    $reviewed_refs = id(new PHUICurtainObjectRefListView())
      ->setViewer($viewer)
      ->setEmptyMessage(pht('None'));

    if ($reviewer_phid) {
      $reviewed_refs = $reviewed_refs->newObjectRefView()
        ->setHandle($handles[$reviewer_phid])
        ->setHighlighted($reviewer_phid === $viewer_phid);
    }

    $curtain->newPanel()
      ->setHeaderText(pht('Reviewer'))
      ->appendChild($reviewed_refs);
