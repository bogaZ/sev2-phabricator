...
final class ManiphestEditEngine
  ...
  protected function buildCustomEditFields($object) {
    ...
    if ($object->getOwnerQAPHID()) {
      $owner_value_qa = array($object->getOwnerQAPHID());
    } else {
      $owner_value_qa = array($this->getViewer()->getPHID());
    }
...
EODOCS
    ...
    $current_key = $this->getEditEngineConfiguration()->getSubType();
    if ($current_key != null) {
      $subtype_value = $current_key;
    }
    ...
    $fields = array(
      ...
      id(new PhabricatorUsersEditField())
        ->setKey('ownerQAPHID')
        ->setAliases(array('ownerQAPHID', 'assignQA', 'assignedQA'))
        ->setLabel(pht('Assigned To QA'))
        ->setBulkEditLabel(pht('Assign to '))
        ->setDescription(pht('User who is responsible for testing the task.'))
        ->setConduitDescription(pht('Reassign the task to QA.'))
        ->setConduitTypeDescription(
          pht('New task owner testing, or `null` to unassign.'))
        ->setTransactionType(ManiphestTaskOwnerQATransaction::TRANSACTIONTYPE)
        ->setIsCopyable(true)
        ->setIsNullable(true)
        ->setSingleValue($object->getOwnerQAPHID())
        ->setCommentActionLabel(pht('QA Assign / Claim'))
        ->setCommentActionValue($owner_value_qa),
      ...
      id(new PhabricatorSelectEditField())
        ...
        ->setBulkEditLabel(pht('Set subtype to'))
        ->setDescription(pht('Subtype of the task.'))
        ->setConduitDescription(pht('Change the subtype of the task.'))
        ->setConduitTypeDescription(pht('New task subtype constant.'))
    ...
    if (!(bool)$this->getIsCreate()) {
      $fields[] = id(new PhabricatorSelectEditField())
       ->setKey('progress')
       ->setLabel(pht('Progress'))
       ->setBulkEditLabel(pht('Set progress task to'))
       ->setDescription(pht('Progress of the task.'))
       ->setConduitDescription(pht('Change the progress of the task.'))
       ->setConduitTypeDescription(pht('New task progress statuses.'))
       ->setTransactionType(ManiphestTaskProgressTransaction::TRANSACTIONTYPE)
    ...
    if (ManiphestTaskPointsQA::getIsEnabled()) {
      $fields[] = id(new PhabricatorPointsEditField())
        ->setKey('pointsQA')
        ->setLabel('Story Points QA')
        ->setBulkEditLabel('Change Story Points QA')
        ->setDescription(pht('Point value of the task for QA.'))
        ->setConduitDescription(pht('Change the task QA point value.'))
        ->setConduitTypeDescription(pht('New task QA point value.'))
        ->setTransactionType(ManiphestTaskPointsQATransaction::TRANSACTIONTYPE)
        ->setIsCopyable(true)
        ->setValue($object->getPointsQA())
        ->setIsRequired(true)
        ->setCommentActionLabel('Change Story Points QA');
    }
    ...
    $conpherence_type = ManiphestTaskHasConpherenceThreadEdgeType::EDGECONST;
  ...
  protected function newEditResponse(
    ...
    switch ($response_type) {
      ...
      default:
        // If it was a request from lobby room
        // redirect back to edge association controller
