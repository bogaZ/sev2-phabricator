...
/* -(  PhabricatorConduitResultInterface  )---------------------------------- */
  ...
  public function getFieldSpecificationsForConduit() {
    return array(
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('title')
        ->setType('string')
        ->setDescription(pht('The title of question.')),
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('status')
        ->setType('string')
        ->setDescription(pht('The status of the question.')),
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('content')
        ->setType('string')
        ->setDescription(pht('The content of question.')),
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('answer')
        ->setType('string')
        ->setDescription(pht('The answer of question.')),
    );
  }

  public function getFieldValuesForConduit() {
    $answers = array();
    $answer_query = id(new PonderAnswerQuery())
      ->setViewer(PhabricatorUser::getOmnipotentUser())
      ->withQuestionIDs(array($this->getID()))
      ->execute();

    if ($answer_query) {
      foreach ($answer_query as $value) {
        $answer['title'] = $value->getContent();
        $answers[] = $answer;
      }
    } else {
      $answers = [];
    ...
    return array(
      ...
      'answerSummary' => $this->getAnswerWiki(),
      'answers' => $answers,
    );
  }
  ...
  public function getConduitSearchAttachments() {
    return array(
      id(new PonderAnswerSearchEngineAttachment())
        ->setAttachmentKey('answers'),
    );
  }
