...
class PhabricatorApplicationTransactionView extends AphrontView {
  ...
  protected function renderTransactionReaction(
    PhabricatorApplicationTransaction $xaction) {

    $viewer = $this->getViewer();
    $engine = $this->getOrBuildEngine();
    $tokens_given = id(new PhabricatorTokenGivenQuery())
      ->withObjectPHIDs(array($xaction->getPHID()))
      ->setViewer($viewer)
      ->execute();
  ...
  private function renderEvent(
    ...
    $reactions = $this->renderTransactionReaction($xaction);
    if ($reactions) {
      $event->setTokenBar($reactions);
    }
