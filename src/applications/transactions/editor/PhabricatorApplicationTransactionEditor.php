...
abstract class PhabricatorApplicationTransactionEditor
  ...
  private function setMention(
    ...
    $parsed_comment = $engine->markupText($message);
    if ($parsed_comment instanceof PhutilSafeHTML) {
      $parsed_comment = $parsed_comment->getHTMLContent();
    }
    $matches = array();
    preg_match_all('/href="\/p\/([a-zA-Z0-9]+)\/"/',
      $parsed_comment,
      $matches);
    $usernames = array_unique($matches[1]);

    if (count($usernames)) {
      $asd = id(new PhabricatorMention())
      ...
      if (count($users)) {
        foreach ($users as $user) {
          $phid = $user->getPHID();
          id(new PhabricatorMentionMentioned())
            ->setMentionID($asd->getID())
            ->setUserPHID($phid)
            ->save();
        }
      }
      ...
      if (count($xactions) > 0) {
        ...
        if ($comment->getComment() !== null) {
          $this->setMention(
            ...
            $comment->getAuthorPHID(),
            $comment->getObjectPHID());
        }
      }
