...
final class PhabricatorStorageManagementAPI extends Phobject {
  ...
  public static function getPatchStatusTable() {
    return sev2table(self::TABLE_STATUS);
  }

  public static function getHostStateTable() {
    return sev2table(self::TABLE_HOSTSTATE);
  }
  ...
  public function getWorkspace() {
    $ws = $this->workspace;
    if (empty($ws)) {
      return PhabricatorEnv::getEnvConfig('sev2.workspace');
    }
    return $ws;
  }
  ...
  public function getDatabaseList(array $patches, $only_living = false) {
    ...
    foreach ($patches as $patch) {
      ...
      if ($patch->getType() == 'sql'
          && strpos($patch->getName(), 'base_') !== false) {
        $list[] = $this->getNamespace().str_replace(
          array('base', '.sql'), '', basename($patch->getName()));
      }
    }
  ...
  public function getAppliedPatches() {
    try {
      ...
      // Try with sev2app table before we give up
      try {
        $applied = queryfx_all(
          $this->getConn('meta_data'),
          'SELECT patch FROM %T',
  ...
  public function applyPatchSQL($sql) {
    ...
    foreach ($queries as $query) {
      ...
      $query = str_replace('{$WORKSPACE}', $this->getWorkspace(), $query);
