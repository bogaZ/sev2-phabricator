...
/* -(  Edit View  )---------------------------------------------------------- */
  ...
  protected function newEditField() {
    ...
    if (strpos($this->getFieldKey(), 'std:maniphest:') !== false) {
      $comment_action = $this->commentType();
    } else {
  ...
  /**
   * Filter custom column type so it can generate
   * comment action based on type
   */
  ...
  protected function commentType() {
    switch ($this->getFieldType()) {
      case 'link':
      case 'text':
        return id(new PhabricatorEditEngineTextCommentAction());
      case 'select':
        $options = $this->getFieldConfigValue('options', array());
