...
final class PhabricatorMainMenuView extends AphrontView {
  ...
  private function renderPhabricatorLogo() {
    ...
    // Almost the end of Phab here :(
    if (id(new Lobby())->setViewer($this->getViewer())->allowedTojoin()) {

      require_celerity_resource('phabricator-lobby-menu-css');

      if ($this->controller instanceof PhabricatorHomeMenuItemController) {
        return;
      } else {
        return id(new PHUIButtonView())
          ->setTag('a')
          ->setHref('/')
          ->setIcon('fa-arrow-left')
          ->addClass('phabricator-main-menu-brand lobby-back-menu')
          ->setNoCSS(true)
          ->setDropdown(false)
          ->setAuralLabel(pht('Back to Lobby'));
      }
    }
