...
final class PHUITimelineEventView extends AphrontView {
  ...
  public function render() {
    ...
    // Reaction
    $inner_content_class = 'phui-timeline-group';
    $tokenview = null;
    if (!empty($this->tokenBar)) {
      $inner_content_class .= ' phui-box-border';
      $tokenview = phutil_tag(
        'div',
          array(
            'class' => 'phui-feed-token-bar mmb',
          ),
        $this->tokenBar);
    }
  ...
  private function getMenuItems($anchor) {
    ...
    if ($this->getIsNormalComment()) {
      ...
      $items[] = id(new PhabricatorActionView())
        ->setIcon('fa-smile-o')
        ->setHref('/transactions/reaction/'.$xaction_phid.'/')
        ->setName(pht('Reaction'))
        ->addSigil('transaction-raw')
        ->setMetadata(
          array(
            'anchor' => $anchor,
